...
ENGINES = [
    ...
    {
        "name":   "Web Hub",
        "sub":    "HTML/CSS/JS · Neon Maze browser game",
        "color":  PINK,
        "action": "web",
...
def build_ui():
    ...
    tk.Label(hdr, text="// GAME ENGINE LAUNCHER  II //", font=("Courier New", 9), bg=BG2, fg=DIM).pack(pady=(2,0))
    ...
    for eng in ENGINES:
        card = tk.Frame(cards_frame, bg=CARD, padx=14, pady=12,
                        highlightbackground=eng["color"], highlightthickness=1)
        card.pack(fill=tk.X, pady=5)

        left = tk.Frame(card, bg=CARD)
        left.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        tk.Label(left, text=eng["name"], font=("Courier New", 12, "bold"),
                 bg=CARD, fg=eng["color"], anchor="w").pack(fill=tk.X)
        tk.Label(left, text=eng["sub"], font=("Courier New", 9),
                 bg=CARD, fg=DIM, anchor="w").pack(fill=tk.X)
        tk.Label(left, text=eng["badge"], font=("Courier New", 8),
                 bg=CARD, fg="#2a1a4a", anchor="w", wraplength=380, justify="left").pack(fill=tk.X, pady=(3,0))

        btn = tk.Button(
            card, text="LAUNCH",
            font=("Courier New", 9, "bold"),
            bg=CARD, fg=eng["color"],
    ...
    tk.Label(root, text="Angela Hudson  ·  PixelForge  ·  2026",
             font=("Courier New", 8), bg=BG2, fg=DIM, pady=7).pack(fill=tk.X)
